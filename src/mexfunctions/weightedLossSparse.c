#include "mex.h"
#include <string.h>
double singleLoss(double* x, double* y,double* W,int index, int numRow,mwIndex* ir_X, mwIndex* jc_X, mwIndex* ir_Y, mwIndex* jc_Y)
{
	// x*y'
	double obj=0;
    double obj1=0;
    ...
    for (i=start;i<over;i++) // find a column
    {
        ...
        //////////////////////////////////////
        int base=posR*numRow;
        double weight=W[base+posR];
...
{   
	...
	for (i=0;i<numCol;i++)
	{
	   out[i]=singleLoss(X, Y,W,i,numRow,ir_X,jc_X,ir_Y,jc_Y);
       //printf("id=%d\n",omp_get_thread_num()); 
	}
}
